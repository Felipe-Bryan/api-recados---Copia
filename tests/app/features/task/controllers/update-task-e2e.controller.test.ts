...
describe('Testando a rota que atualiza um recado', () => {
  ...
  test('Deveria retornar erro 404 caso não encontre o recado', async () => {
    ...
    const result = await supertest(sut).put(`${route}/wrongId`).send({});
  ...
  test('Deveria retornar sucesso e alterar a propriedade informada(detail) caso encontre o recado', async () => {
    const user = await setLoggedUser('any');
    const route = createRoute(user.id);
    ...
    const result = await supertest(sut).put(`${route}/${userTasks.data[0].id}`).send({
      detail: 'new_detail',
    });
    ...
    expect(result).toHaveProperty('body.ok');
    expect(result.body.msg).toBe('Task updated');
    expect(result.body.ok).toBe(true);

    expect(result).toHaveProperty('body.data');
    expect(result.body.data).toHaveProperty('id');
    expect(result.body.data).toHaveProperty('detail', 'new_detail');
    expect(result.body.data).toHaveProperty('description', 'any_description');
  });

  test('Deveria retornar sucesso e alterar a propriedade informada(description) caso encontre o recado', async () => {
    const user = await setLoggedUser('any');
    const route = createRoute(user.id);
    await supertest(sut).post(`${route}`).send({
      detail: 'any_detail',
    ...
    expect(result.body.data).toHaveProperty('description', 'new_description');
  });
});
